...
from Structure.spn_ensemble import CombineSPN
from rspn.algorithms.ranges import NominalRange, NumericRange
from rspn.rspn import RSPN
from rspn.structure.leave import IdentityNumericLeaf, identity_distinct_ranges, categorical_distinct_ranges, \
...
class AQPSPN(CombineSPN, RSPN):
    ...
    def learn(self, train_data, rdc_threshold=0.15, rdc_threshold_high=0.3, min_instances_slice=1, max_sampling_threshold_cols=10000,
              max_sampling_threshold_rows=100000, bloom_filters=False, table_condition=None, attr_condition=None):
        ...
        RSPN.learn(self, train_data, rdc_threshold=rdc_threshold, rdc_threshold_high=rdc_threshold_high, min_instances_slice=min_instances_slice,
    ...
    def evaluate_indicator_expectation(self, indicator_expectation, standard_deviations=False,
                                       gen_code_stats=None, outer_join_table=None):
        """
        ...
        for (table, condition) in conditions:
            ...
            if table_obj.table_nn_attribute in condition:
                ...
                # column can become NULL
                    ...
                    ###
                    ranges[:, attribute_index] = NominalRange([1], attr=full_nn_attribute_name, table=table)
                    ...
                    ranges[:, attribute_index] = NominalRange([0], attr=full_nn_attribute_name, table=table)
                    continue
                else:
                ...
                if self.meta_types[attribute_index] == MetaType.DISCRETE:
                    ...
                    if values == list(range(min_value, max_value + 1)):
                        ranges = _adapt_ranges(attribute_index, max_value, ranges, inclusive=True, lower_than=True,
                                               attr=matching_column, table=table)
            ...
            if self.meta_types[attribute_index] == MetaType.DISCRETE:
                ...
                if '=' in condition:
                    ...
                    if group_by_columns_merged is None or matching_column not in group_by_columns_merged:
                        ranges[:, attribute_index] = NominalRange([val_dict[literal]], attr=matching_column, table=table)
                    else:
                        ...
                        for idx in range(len(ranges)):
                            ...
                            ranges[idx, attribute_index] = NominalRange([literal], attr=matching_column, table=table)
                    ...
                    literal_list = _literal_list(condition)
                    single_range = NominalRange([val_dict[literal] for literal in literal_list], attr=matching_column,
                                                table=table)
                    if all([single_range is None for single_range in ranges[:, attribute_index]]):
                        ranges[:, attribute_index] = single_range
                    else:
                ...
                if '<=' in condition:
                    ...
                    if group_by_columns_merged is None or matching_column not in group_by_columns_merged:
                        ranges[:, attribute_index] = NumericRange([[literal, literal]], attr=matching_column, table=table)
                    else:
                        matching_group_by_idx = group_by_columns_merged.index(matching_column)
                        assert literal == group_by_tuples[0][matching_group_by_idx]
                        for idx in range(len(ranges)):
                            literal = group_by_tuples[idx][matching_group_by_idx]
                            ranges[idx, attribute_index] = NumericRange([[literal, literal]], attr=matching_column, table=table)
...
def _adapt_ranges(attribute_index, literal, ranges, inclusive=True, lower_than=True, attr=None, table=None):
