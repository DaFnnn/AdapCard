...
from rspn.structure.leave import IdentityNumericLeaf, identity_expectation, Categorical, categorical_likelihood_range, \
...
class RSPN:
    ...
    ###
    # add rdc threshold and condition
    def learn(self, train_data, rdc_threshold=0.15, rdc_threshold_high=0.3, min_instances_slice=1, max_sampling_threshold_cols=10000,
              max_sampling_threshold_rows=100000, no_compression_scopes=None, table_condition=None, attr_condition=None):
        # min_instances_slice: the minimum number of instances required to slice a dataset
        # build domains (including the dependence analysis)
        domain_start_t = time.perf_counter()
        # The build_ds_context function is called to build the domains for the dataset,
        # which includes analyzing the data and identifying dependencies between variables.
        # The resulting ds_context object contains information about the domain,
        # such as variable types and null values.
    ...
    def _indicator_expectation(self, feature_scope, identity_leaf_expectation=None, inverted_features=None,
        ...
        if hasattr(self, 'use_generated_code') and self.use_generated_code and not force_no_generated:
            full_result = expectation(self.mspn, feature_scope, inverted_features, range_conditions,
                                      ...
                                      gen_code_stats=gen_code_stats, outer_join_table=outer_join_table)
        else:
            full_result = expectation(self.mspn, feature_scope, inverted_features, range_conditions,
                                      node_expectation=_node_expectation, node_likelihoods=_node_likelihoods_range,
