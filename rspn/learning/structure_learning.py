...
def get_next_operation(min_instances_slice=100, min_features_slice=1, multivariate_leaf=False):
    def next_operation(
            ...
            table_condition=None, attr_condition=None
    ):

        ###
        # condition split
        # if table_condition is not None:
        #     return Operation.SPLIT_TABLE, (table_condition, attr_condition)
        #
        # if attr_condition is not None:
        #     return Operation.SPLIT_ROWS_CONDITION, attr_condition
        ...
        if ncols_zero_variance > 0:
            if ncols_zero_variance == data.shape[1]:
                if multivariate_leaf:
                    return Operation.CREATE_LEAF, None
                ###
                # elif data.shape[0] > 10000 and data.shape[1] > 1:
                #     return Operation.SPLIT_ROWS, None
                # elif data.shape[0] > 1000 and data.shape[1] > 2:
                #     return Operation.SPLIT_COLUMNS_TEST, None
    ...
    while tasks:
        ...
        if operation == Operation.REMOVE_UNINFORMATIVE_FEATURES:
            ...
            if op_params is None:
                op_params = 2
            data_slices, cluster_centers = split_rows(local_data, ds_context, scope, op_params)
        ...
        elif operation == Operation.SPLIT_COLUMNS_TEST:

            from rspn.learning.rspn_learning import get_split_cols_RDC_py_test

            split_start_t = perf_counter()

            split_cols_test = get_split_cols_RDC_py_test()
            data_slices = split_cols_test(local_data, ds_context, scope)
            ...
            from rspn.structure.leaves.piecewise.PiecewiseLinear import create_piecewise_leaf
            result_nodes = pool.starmap(create_leaf, local_children_params)
            # result_nodes = pool.starmap(create_piecewise_leaf, local_children_params)
        ...
        ###
        # naive factorization with weighted sampling
        ...
        #     for i in range(2):
        #         node.children.append(None)
        #         node.weights.append(0.001 + i * 0.998)
        #
        #     ###
        #     #sample node with weights 0.5
        #     sample_node = Sample(data=local_data, scope=scope)
        #     sample_node.cardinality = local_data.shape[0]
        #
        #     # node.scope.extend(scope)
        #
        #     sample_node.data = local_data
        #
        #     node.children[0] = sample_node
        #
        #     ###
        #     # naive factorization node with weights 0.5
        #     naive_node = Product()
        #     naive_node.cardinality = local_data.shape[0]
        #
        #     naive_node.scope.extend(scope)
        #     node.children[1] = naive_node
        #
        #     local_tasks = []
        #     local_children_params = []
        #     split_start_t = perf_counter()
        #     for col in range(len(scope)):
        #         naive_node.children.append(None)
        #         local_tasks.append(len(naive_node.children) - 1)
        #         child_data_slice = data_slicer(local_data, [col], num_conditional_cols)
        #         local_children_params.append((child_data_slice, ds_context, [scope[col]]))
        #
        #     from rspn.structure.leaves.piecewise.PiecewiseLinear import create_piecewise_leaf
        ...
        #
        #     for child_pos, child in zip(local_tasks, result_nodes):
        #         naive_node.children[child_pos] = child
        #
        #     split_end_t = perf_counter()
        #
        #     logging.debug(
        ...
        ###
        # change naive factorization with chow liu tree or piecewise regression
        # elif operation == Operation.NAIVE_FACTORIZATION:
        #     from rspn.structure.leaves.piecewise.PiecewiseLinear import create_piecewise_leaf
        #
        #     leaf_start_t = perf_counter()
        #     node = create_piecewise_leaf(local_data, ds_context, scope)
        #     parent.children[children_pos] = node
        #     leaf_end_t = perf_counter()
        #
        #     logging.debug(
        #         "\t\t created piecewise leaf {} for scope={} (in {:.5f} secs)".format(
        ...
        #     )
        ...
        elif operation == Operation.BAGGING:
            import pandas as pd

            n = 2
            df = pd.DataFrame(local_data)
            bagged_data = [df.sample(frac=min(1.0, 0.8), replace=False) for i in range(n)]
            ...
            cardinality = 0
            for i in range(n):
                cardinality += len(bagged_data[i])

            for i in range(n):
                node.children.append(None)
                node.weights.append(len(bagged_data[i]) / cardinality)
                tasks.append((bagged_data[i].values, node, len(node.children)-1, scope, False, False))
            node.cardinality = local_data.shape[0]

        elif operation == Operation.SAMPLING:
            node = Sample(data=local_data, scope=scope)
            node.cardinality = local_data.shape[0]
            ...
            parent.children[children_pos] = node
        ###
        # elif operation == Operation.SPLIT_TABLE:
        #
        #     node = Product()
        #     node.cardinality = local_data.shape[0]
        #
