...
def read_table_csv(table_obj, csv_seperator=','):
    ...
    """
    df_rows = pd.read_csv(table_obj.csv_file_location, header=0, escapechar='\\', encoding='utf-8', quotechar='"',
                          sep=csv_seperator)
    ...
    ###
    # change for kddcup98
    # if 'kdd' in table_obj.table_name:
    #     df_rows = pd.read_csv(table_obj.csv_file_location, header=0, escapechar='\\', encoding='utf-8',
    #                           quotechar='"',
    #                           sep=csv_seperator)
    # else:
...
###
def find_relationship_two(start_table, end_table, relationship_list):
    for relationship_obj in relationship_list:
        if relationship_obj.start == start_table and relationship_obj.end == end_table :
            return relationship_obj
        elif relationship_obj.start == end_table and relationship_obj.end == start_table :
            return relationship_obj
    return None
...
def prepare_single_table(schema_graph, table, path, max_distinct_vals=100000, csv_seperator=',',
    ...
    # change for kdd
    # if 'kdd' in table_obj.table_name:
    #     relevant_attributes = table_data.columns.values.tolist()
    #     with open('./imdb-benchmark/kddcup98/kdd_col_names.txt', 'w') as f:
    #         f.write(str(relevant_attributes))
    # else:
    #     relevant_attributes = [x for x in table_obj.attributes if x not in table_obj.irrelevant_attributes]
    ...
    for rel_attribute in relevant_attributes:
        ...
        if table_data.dtypes[attribute] == object:
            ...
            # ### remove blank # take care of blank in strings
            # tmp = distinct_vals.tolist()
            # for i in range(len(tmp)):
            #     if isinstance(tmp[i], str):
            #         tmp[i] = tmp[i].strip()
            # distinct_vals = np.array(tmp)
    ...
    # ### change for kdd
    # if 'kdd' in table:
    #     del_cat_attributes = [rel_attribute for rel_attribute in del_cat_attributes]
    # else:
    #     del_cat_attributes = [table + '.' + rel_attribute for rel_attribute in del_cat_attributes]
    ...
    if len(table_data) < max_table_data:
        ...
        ###
        # seed
        table_data.sample(max_table_data, random_state=11).to_hdf(path, key='df', format='table')

    # add table parts without join partners
    # 这段代码是用来将当前输入表中未连接的记录进行索引，以及将它们存储为单独的 Pandas dataframe 表格。
    # 具体来说，获取当前关系图中所有与该表存在输入关系的关系对象。然后，
    # 从邻居表中获取主键并在当前输入表属性和邻居表属性之间添加连接。接下来，
    # 使用左联接查找当前表中未连接的记录，并使用列名参数保留相关属性。最后，如果找到未连接记录，则记录元数据并使用HDFS
    # 格式将未连接记录存储在提供的路径下。
    ###
...
def prepare_all_tables(schema_graph, path, csv_seperator=',', max_table_data=20000000):
    ...
    with open(path + '/meta_data_{}.pkl'.format(schema_graph.tables[0].table_name), 'wb') as f:
        pickle.dump(meta_data, f, pickle.HIGHEST_PROTOCOL)
    prep_end_t = perf_counter()

    # with open(path + '/build_time_hdf_{}.txt'.format(schema_graph.tables[0].table_name), "w") as text_file:
    # ### for collected datasets
