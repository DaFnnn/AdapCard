...
from DataPreparation.prepare_single_tables import find_relationships, find_relationship_two
from Learning.utils import create_random_join
# from Learning.rdc_based import max_rdc
...
def prob_round(x):
    ...
    这段代码实现了一个基于随机概率的浮点数四舍五入函数 prob_round，
    可以确保在需要对数据进行加噪处理时，数据的随机性与可靠性之间平衡。
    """
...
class JoinDataPreparator:
    ...
    def generate_n_samples(self, sample_size, post_sampling_factor=30, single_table=None, relationship_list=None,
        ...
        if len(df_full_samples) > sample_size:
            ###
            # seed
            return df_full_samples.sample(sample_size, random_state=11), meta_types, null_values, full_join_size
        return df_full_samples, meta_types, null_values, full_join_size


    def generate_n_samples_with_incremental_part(self, sample_size=100000000, final_sample_size=1000000, post_sampling_factor=30, single_table=None, relationship_list=None,
        ...
        ###
        # for full join size
        full_join_size = len(df_full_samples)
        # if len(df_full_samples) > sample_size:
        ###
        if len(df_full_samples) > final_sample_size:
            ### seed
            # df_full_samples = df_full_samples.sample(sample_size, random_state=11)
            df_full_samples = df_full_samples.sample(final_sample_size, random_state=11)
        ...
        # return df_learn_samples, df_inc_samples, meta_types, null_values, len(df_learn_samples)
    ...
    def generate_join_sample(self, single_table=None, relationship_list=None, min_start_table_size=1, sample_rate=1,
        ...
        if single_table is not None:
            ...
            if sample_rate < 1:
                ### seed
                df_samples = df_samples.sample(prob_round(len(df_samples) * sample_rate), random_state=11)
            ...
            ###
            # temp sample for calculating join size
            # temp_sample = df_samples
            if sample_rate < 1:
                ### seed
                df_samples = df_samples.sample(prob_round(len(df_samples) * sample_rate), random_state=11)
            ...
            while len(todo_relationships) > 0:
                if len(df_samples) > max_intermediate_size:
                    ### seed
                    df_samples = df_samples.sample(max_intermediate_size, random_state=11)
                ...
                if outgoing:
                    ...
                    # ####
                    # if len(next_table_data) > 10000000:
                    #     next_table_data = next_table_data.sample(n=10000000)
                    ...
                    # ####
                    # if len(df_samples) > 10000000:
                    #     df_samples = df_samples.sample(n=10000000)
                    ...
                    if edge_information is not None:
                        ...
                        if sample_rate * start_table_sample_rate < 1:
                            ...
                            wo_join_partners = wo_join_partners.sample(
                                prob_round(len(wo_join_partners) * sample_rate * start_table_sample_rate), random_state=11)
    ...
    ###
    # find different level correlations
    # def find_correlations(self, schema, sample_size, postsampling_factor = 30, rdc_threshold_high = 0.7, rdc_threshold_low = 0.3):
    #     relationship_list = schema.relationships
    #
    #     i = 0
    #     for relationship_obj in relationship_list:
    #         start_table = relationship_obj.start
    #         start_table_data = self._get_table_data(self.table_meta_data[start_table]['hdf_path'], start_table)
    #         end_table = relationship_obj.end
    #         end_table_data = self._get_table_data(self.table_meta_data[end_table]['hdf_path'], end_table)
    #
    #         # set indexes to make pandas join fast
    #         left_attribute = relationship_obj.end + '.' + relationship_obj.end_attr
    #         right_attribute = relationship_obj.start + '.' + relationship_obj.start_attr
    #
    #         start_table_data = start_table_data.set_index(left_attribute, drop=False)
    #         end_table_data = end_table_data.set_index(right_attribute, drop=False)
    #
    #         start_table_data.index.name = None
    #
    #         merged_table_data = start_table_data.merge(end_table_data, how='left', right_index=True,
    #                                               left_on=left_attribute)
    #         rdc_attribute_dict = dict()
    #
    #
    #         # relationship_list_one = []
    #         # relationship_list_one.append(relationship_obj.identifier)
    #         # df_samples, meta_types, _ = self.generate_join_sample(
    #         #     relationship_list=relationship_list_one,
    #         #     min_start_table_size=1, sample_rate=1.0,
    #         #     drop_redundant_columns=True,
    #         #     max_intermediate_size=sample_size * postsampling_factor)
    #         rdc_value = self.max_rdc(schema, start_table, end_table, df_samples, meta_types, rdc_attribute_dict)
    #
    #         schema.relationships[i].rdc_score = rdc_value
    #         # schema.relationships[i].correlation_level = CorrelationLevel
    #         from DataPreparation.graph_representation import CorrelationLevel
    #         if rdc_value >= rdc_threshold_high:
    #             schema.relationships[i].correlation_level = CorrelationLevel.Strong
    #             schema.strongly_correlated_relationship.append(relationship_obj)
    #         elif rdc_threshold_high > rdc_value > rdc_threshold_low:
    #             schema.relationships[i].correlation_level = CorrelationLevel.Medium
    #             schema.medium_correlated_relationship.append(relationship_obj)
    #         else:
    #             schema.relationships[i].correlation_level = CorrelationLevel.Weak
    #             schema.weakly_correlated_relationship.append(relationship_obj)
    #         i = i + 1
    ...
    #
    #     pairwise_comparisons = [(column_left, column_right) for i, column_left in left_column_names for j, column_right
    #                             in
    #                             right_column_names]
    ...
    ###
    # generate join samples for tables which need to be joined and generate multiplier fields
    def generate_join_sample_for_relationship(self, schema, left_table, right_table, sample_rate=1, drop_redundant_columns=True, \
                              max_intermediate_size=math.inf):
        assert left_table is not None and right_table is not None, "left table and right table should not be none"
        assert find_relationship_two(left_table, right_table, schema.relationships) is not None, "left table and right table should exist join relationship"

        # left_table_data = self._get_table_data(self.table_meta_data[left_table]['hdf_path'], left_table)
        # right_table_data = self._get_table_data(self.table_meta_data[right_table]['hdf_path'], right_table)

        # relationship_list is not None
        todo_relationships = copy.copy(find_relationship_two(left_table, right_table, schema.relationships))

        # start_table, table_set = self._find_start_table(todo_relationships, 1)
        # start_table_sample_rate = self._sampling_rate(start_table)
        start_table_sample_rate = self._sampling_rate(left_table)

        # sample from first table
        logging.debug(f"reading first table '{left_table}'")
        df_samples = self._get_table_data(self.table_meta_data[left_table]['hdf_path'], left_table)
        if sample_rate < 1:
        ...
        if outgoing:
            ...
            # df_samples = df_samples.merge(next_table_data, how='left', right_index=True,
            #                               left_on=right_attribute)
            df_samples = df_samples.merge(next_table_data, how='outer', right_index=True,
                                          left_on=right_attribute)
        ...
        ###
        # save joined hdf files
        df_samples.to_hdf(f'{self.hdf_path}/{left_table}_joined_{relationship_obj.end}_sampled.hdf', key='df',
                          format='table')
...
def prepare_sample_hdf(schema, hdf_path, max_table_data, sample_size):
    ...
    meta_data_path = hdf_path + '/meta_data_{}.pkl'.format(schema.tables[0].table_name)
    ...
    ###
    # prep.find_correlations(schema, max_table_data)
        ...
        for relationship_obj in schema.relationships:
            if (relationship_obj.start in sampled_tables and not relationship_obj.end in sampled_tables) or (
                ...
                if len(next_table_data) > sample_size:
                    ### seed
                    next_table_data = next_table_data.sample(sample_size, random_state=11)
    ...
    # different meta datass
    # with open(hdf_path + '/meta_data_sampled.pkl', 'wb') as f:
    # ### for collected datasets
    with open(hdf_path + '/meta_data_{}_sampled.pkl'.format(schema.tables[0].table_name), 'wb') as f:
